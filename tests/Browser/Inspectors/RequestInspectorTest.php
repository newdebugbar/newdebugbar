...
it('keeps request evidence aligned and middleware accessible across viewports', function (int $width, int $height, string $theme) {
    ...
    $profile['inspectors']['request']['payload']['url'] = 'http://newdebugbar.test'.$profile['inspectors']['request']['payload']['path'];
    $profile['inspectors']['request']['payload']['action'] = 'App\\Http\\Controllers\\'.str_repeat('LongJourneyWorkspace', 6).'Controller@show';
    $profile['inspectors']['request']['payload']['middleware'] = array_map(
    ...
    $page->assertAttribute('#newdebugbar', 'data-ndb-theme', $theme)
        ...
        ->assertScript(<<<'JS'
            (() => {
                const popover = document.querySelector('[data-ndb-request-middleware-popover]');
                const target = [...document.querySelectorAll('[data-ndb-request-step] h3, [data-ndb-request-step] dt, [data-ndb-inspector-heading]')]
                    .find((element) => {
                        const bounds = element.getBoundingClientRect();
