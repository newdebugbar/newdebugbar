    ...
    $page
        ...
        ->assertScript(<<<'JS'
            (() => {
                ...
                const loadedInspector = document.querySelector('[data-ndb-loaded-inspector="models"]');
                const stage = document.querySelector('[data-ndb-inspector-stage]');
                const inspectorBody = document.querySelector('[data-ndb-inspector-body]');
                ...
                const checks = {
                    ...
                    horizontalPadding: getComputedStyle(loadedInspector).paddingLeft === '0px'
                        && getComputedStyle(loadedInspector).paddingRight === '0px',
                    ...
                    fullHeight: getComputedStyle(content).display === 'flex'
                        ...
                        && getComputedStyle(inspectorBody).display === 'flex'
                        && Math.abs(
                            workspace.getBoundingClientRect().bottom
                            - loadedInspector.getBoundingClientRect().bottom
                            + Number.parseFloat(getComputedStyle(loadedInspector).paddingBottom)
                        ) <= 1,
...
it('adapts the model list and details into a mobile drill in flow', function () {
    ...
    $page
        ...
        ->assertScript(<<<'JS'
            (() => {
                const content = document.querySelector('[data-ndb-inspector-content]');
                const panel = document.querySelector('[data-ndb-inspector-panel="models"]');
...
it('summarizes writes and shows completed operations without lifecycle noise', function () {
    visit('/profiled-models?changes=1')
        ->click('[data-ndb-window-controls="compact"] [data-ndb-window-action="expand"]')
        ->click('[data-ndb-select-inspector="models"]')
        ->assertScript('document.querySelector("[data-ndb-select-inspector=models]").textContent.trim().endsWith("48")')
        ->assertScript(<<<'JS'
            JSON.stringify([...document.querySelectorAll('[data-ndb-model-group]')]
                .map((group) => [
...
it('shows a useful empty model state', function () {
    ...
    DebugBarBrowser::selectInspectorViaPalette($page, 'models');
