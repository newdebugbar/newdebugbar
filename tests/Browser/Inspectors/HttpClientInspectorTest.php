...
it('filters, selects, and inspects outbound HTTP evidence', function () {
    visit('/profiled-http-client-rich')
        ...
        ->click('[data-ndb-select-inspector="http_client"]')
        ...
        ->assertScript(<<<'JS'
            (() => {
                ...
                const inspectorNavigation = document.querySelector('#newdebugbar-inspector-navigation');
                ...
                return getComputedStyle(workspace).display === 'grid'
                    ...
                    && Math.abs(workspace.getBoundingClientRect().left - inspectorNavigation.getBoundingClientRect().right) <= 1
    ...
    visit('/profiled-http-client-rich')
        ...
        ->assertScript(<<<'JS'
            (() => {
                ...
                const workspace = document.querySelector('[data-ndb-http-client-workspace]');
                const loadedInspector = document.querySelector('[data-ndb-loaded-inspector="http_client"]');
                const loadedStyle = getComputedStyle(loadedInspector);
...
it('shows a useful empty HTTP client state', function () {
    ...
    DebugBarBrowser::selectInspectorViaPalette($page, 'http_client');
