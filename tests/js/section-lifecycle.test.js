...
const profile = { ...summary, id: 'first', sections: sections.map((key) => ({ key, label: key })) };
const encode = (payload) => Buffer.from(JSON.stringify(payload), 'utf8').toString('base64');

test('section mounts accept refreshed payloads and stop reading them after replacement or destruction', () => {
  for (const section of sections) {
    const { shell, state } = sectionHarness(section, profile, runtime());
    let payload = null;
    let reads = 0;
    state.$root = {
    ...
    state.init();
    payload = section === 'livewire' ? { components: [], activity_records: [] } : [];
    shell.refreshSection();
    assert.equal(state.initialized, true, section);
    shell.refreshSection();
    shell.summary = { ...profile, id: 'second' };
    const previousReads = reads;
    shell.refreshSection();
    state.refresh();
    assert.equal(reads, previousReads, `${section} ignores a different profile`);
    shell.summary = profile;
    state.destroy();
    state.refresh();
    shell.refreshSection();
    assert.equal(reads, previousReads, `${section} detaches on destruction`);
  }
});

test('a replaced section cannot detach its replacement through a different Alpine scope proxy', () => {
  const { shell, state: first } = sectionHarness('mail', profile, runtime());
  const second = shell.createSection('mail');
  let refreshed = 0;
  ...
  shell.refreshSection();
  assert.equal(refreshed, 1);
  assert.deepEqual(shell.createSection('request'), {});
});

test('section instances retain live getters and decode Unicode without sharing records or controls', () => {
  const { shell, state } = sectionHarness('mail', profile, runtime());
  const next = shell.createSection('mail');
...
test('same-profile query refresh preserves selection, scrolling, and pending or completed EXPLAIN evidence', () => {
  const { state } = sectionHarness('queries', profile, runtime());
  const records = () => [
    {
      key: 'kyoto',
      executions: [
  ...
  state.receiveQueryExplain({ profileId: profile.id, execution: 2, explain: { rows: [{ detail: 'SCAN trips' }] } });
  state.initializeQueries(records());
...
test('view refresh retains loaded render data and rejects a late response from another profile', async () => {
  const { shell, state } = sectionHarness('views', profile, runtime());
  ...
  const next = shell.createSection('views');
...
test('closing Timeline cancels observation and prevents late pagination callbacks from reopening it', async () => {
  ...
  const { shell, state } = sectionHarness('timeline', profile, browser);
...
test('Livewire resumes its display subscription without clearing captured page history', () => {
  ...
  const { shell, state } = sectionHarness('livewire', profile, browser, [], 20, trace);
  state.$nextTick = (callback) => callback();
  state.init();
