...
test('query workspace filters selects and keeps explain evidence scoped to the active execution', async () => {
  ...
  const { state, shell } = sectionHarness('queries', { ...summary, query_count: 4 }, browser);
  const appended = [];
  const scrolls = [];
  let detailFocused = 0;
  let rowFocused = 0;
...
test('EXPLAIN ignores success and failure from a replaced profile with the same execution', async () => {
  ...
  const { state, shell } = sectionHarness('queries', { ...summary, id: firstId }, runtime());
  ...
  state.destroy();
  const next = shell.createSection('queries');
