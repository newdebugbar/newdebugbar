...
test('mail defaults to all and preview while keeping a visible message selected', () => {
  ...
  const { state, shell } = inspectorHarness('mail', summary, browser);
  let detailResets = 0;
...
test('mail preview keeps desktop and mobile viewport widths inside a narrow canvas', () => {
  ...
  try {
    const { state, shell } = inspectorHarness('mail', summary, runtime());
    const canvas = {
      clientWidth: 320,
      style: {
...
test('mail preview follows canvas resizes and cleans up its observer', () => {
  ...
  try {
    ...
    const { state, shell } = inspectorHarness(
      'mail',
      { ...summary, inspectors: [...summary.inspectors, { key: 'mail', label: 'Mail' }] },
      browser,
    );
    const scrolls = [];
