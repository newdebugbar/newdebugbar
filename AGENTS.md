...
- Once the required checks pass, stop verification unless further changes, failures, or unresolved concerns justify another check. For small changes, use an equivalent check when a new test would only repeat the implementation.
- Treat intermittent test failures as unresolved until their cause is explained and fixed. A passing rerun alone does not clear a failure for pushing or merging. Verify the fix under the conditions that exposed it; do not add retries or weaken assertions to make CI green.
- For browser interactions, wait for the selected inspector's deferred content to finish loading before using its controls. Return observable values from `assertScript` so Pest can wait for rendering to settle; a JavaScript exception bypasses that polling and should not signal a temporary layout mismatch.
