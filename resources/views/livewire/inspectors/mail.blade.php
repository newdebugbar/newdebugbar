...
@php
    $capturedMailItems = $inspector['payload']['items'] ?? [];
    $mailSummary = $inspector['summary'];
    ...
    $mailItems = collect($capturedMailItems)
        ...
        ->map(function (array $item, int $index) use ($formatMailBytes, $profileId): array {
            ...
            $relatedInspector = $isOrigin && $status === 'sent' ? 'mail' : 'queue';
            ...
            return [
                ...
                'related_label' => $isOrigin
                    ? ($relatedInspector === 'mail' ? 'Open worker preview' : 'Open failed worker')
                    : 'Open request',
                'source' => $source,
