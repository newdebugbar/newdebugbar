...
@php
    $capturedNotificationItems = array_values($inspector['payload']['items'] ?? []);
    $notificationSummary = $inspector['summary'];
    $retainedMailIds = collect($profile['inspectors']['mail']['payload']['items'] ?? [])
    ...
    $notificationGroups = collect($capturedNotificationItems)
        ...
        ->map(function ($attempts, int $groupIndex) use ($profileId, $retainedMailIds, $formatNotificationDestinations): array {
            ...
            $relatedInspector = $isOrigin && $hasMailChannel && $status === 'sent'
                ? 'mail'
                : 'notifications';
                    ...
                    @foreach ($notificationGroups as $notification)
                        ...
                        >
                            ...
                            <span
                                ...
                                @class([
                                    'ndb:justify-self-end ndb:text-xs ndb:font-bold',
                            ...
                            <span
                                ...
                                class="ndb:col-start-2 ndb:justify-self-end ndb:text-right ndb:text-xs ndb:font-semibold ndb:tabular-nums ndb:text-zinc-400"
                            >
