...
@php
    $timelineItems = array_values($inspector['payload']['items'] ?? []);
    $timelineInspectors = $inspector['payload']['available_inspectors'] ?? array_values(array_unique(array_column($timelineItems, 'inspector')));
    $timelineSourceInspectors = array_values(array_filter($timelineInspectors, fn ($timelineInspector) => $timelineInspector !== 'request'));
    $timelineDuration = (float) ($inspector['payload']['total_duration_ms'] ?? max(0.001, ...array_column($timelineItems, 'at_ms')));
    $timelineTotal = (int) ($inspector['payload']['total_item_count'] ?? count($timelineItems));
    $timelineMatches = (int) ($inspector['payload']['matching_item_count'] ?? count($timelineItems));
                ...
                >
                            ...
                            <x-slot:filter>
                                ...
                                >
                                    ...
                                    <optgroup label="Source">
                                        ...
                                        @foreach ($timelineSourceInspectors as $timelineInspector)
                                            <option value="{{ $timelineInspector }}">
                                                {{ str($timelineInspector)->replace('_', ' ')->title() }}
                                            </option>
                                        @endforeach
                                    </optgroup>
                                </x-newdebugbar::select-field>
                    ...
                    >
                        ...
                        @foreach ($timelineItems as $item)
                            @php
                                $timelineInspectorLabel = $item['inspector_label'] ?? str($item['inspector'])->replace('_', ' ')->title();
                            ...
                            <button
                                ...
                                data-ndb-timeline-inspector-label="{{ $timelineInspectorLabel }}"
                        ...
                        @if ($inspector['payload']['has_more'] ?? false)
                            <div
                        ...
                        <x-newdebugbar::inspector-detail-header layout="wrap">
                            <x-slot:title>
                                <div class="ndb:min-w-0">
                                    <p
                                        ...
                                        x-text="selectedTimelineItem.inspectorLabel"
                                    ></p>
                                    <h3
                            ...
                            <x-slot:aside>
                                <x-newdebugbar::inspector-action
                                    icon="external-link"
                                    data-ndb-timeline-open-inspector
                                    @click="inspector.selectInspector(selectedTimelineItem.inspector)"
                                >
                                    Open inspector
                                </x-newdebugbar::inspector-action>
                            </x-slot:aside>
