...
@php
    $presenter = app(\NewDebugBar\Presentation\QueryRecordPresenter::class);
    $queryRecords = $presenter->withExplains(
        $inspector['payload']['records'] ?? $presenter->present($inspector['payload'] ?? []),
        $queryExplains,
        $queryExplainErrors,
    );
    $querySummary = is_array($inspector['summary'] ?? null) ? $inspector['summary'] : [];
    $queryFilters = $presenter->filters($queryRecords);
    $queryRetainedCount = $queryFilters['all'][1];
