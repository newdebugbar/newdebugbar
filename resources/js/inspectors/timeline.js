...
export function createTimeline(context) {
  ...
  return {
    ...
    loadNextTimelinePage(wire = this.$wire) {
      ...
      const scopedWire = typeof island === 'function' ? island.call(wire, 'inspector-details') : wire;
    ...
    setTimelineFilter(filter) {
      if (!shell.inspectorKeys.includes(filter) && !['all', 'key'].includes(filter)) return;
    ...
    selectTimelineItem(id) {
      ...
      this.timelineSelected = {
        id,
        inspector: item.dataset.ndbTimelineInspector,
        inspectorLabel: item.dataset.ndbTimelineInspectorLabel,
