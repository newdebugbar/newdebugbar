...
export function createActivity(context) {
  ...
  return {
    ...
    get livewireActivity() {
      ...
      const isCurrentLivewireRequest =
        shell.summary?.request_type === 'livewire' && currentProfileId !== null;
      ...
      const renderEvidence = new Map(
        serverActivity.map((item) => [
          item.id,
          (item.serverRenderIds ?? []).map((id) => byId.get(id)).filter(Boolean),
        ]),
      );
      ...
      const matchingServerEvidence = (item) => {
        ...
        if (item.kind === 'mount')
          return candidates.filter((serverItem) => serverItem.kind === 'mount').slice(0, 1);
      ...
      const browserActivity = this.livewireTrace.activity.map((item) => {
        ...
        const callsite =
          evidence.find((serverItem) => serverItem.callsite)?.callsite ?? item.callsite ?? null;
        ...
        return {
          ...
          serverRenderDurationMs:
            renderDurations.length > 0
              ? renderDurations.reduce((total, duration) => total + duration, 0)
              : null,
      ...
      const unmatchedServerActivity = serverActivity.flatMap((item) => {
        ...
        const renders = (renderEvidence.get(item.id) ?? []).filter(
          (render) => !consumedServerIds.has(render.id),
        );
        renders.forEach((render) => consumedServerIds.add(render.id));
        return [
          {
      ...
      return [...browserActivity, ...unmatchedServerActivity]
        .sort((left, right) => {
          ...
          const leftHasTime =
            left.requestAtMs !== null && left.requestAtMs !== undefined && Number.isFinite(leftAt);
          const rightHasTime =
    ...
    livewireMeaningfulActions(item) {
      return (item?.actions ?? []).filter(
        (action) => !['$commit', '$set', '__dispatch'].includes(action.name),
      );
    },
    ...
    livewireActivityProfileIds(item) {
      return [
        ...new Set(
          (Array.isArray(item?.profileIds) ? item.profileIds : []).filter((id) =>
            PROFILE_PATTERN.test(id ?? ''),
          ),
        ),
      ];
    },
    ...
    livewireActivitySummary(item) {
      ...
      if (item.kind === 'mutation') {
        ...
        if (paths.length > 1) {
          if (failed)
            return `${component} tried to change ${paths.length} properties, but the update did not finish.`;
    ...
    livewireInitialRenderDuration(item) {
      if (item?.initialRenderDurationMs === null || item?.initialRenderDurationMs === undefined)
        return 'Not captured';
