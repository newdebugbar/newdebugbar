  ...
  return {
    ...
    get livewireComponents() {
      ...
      const components = merged
        .map((component) => {
          ...
          while (
            ...
            !seen.has(String(current.parentId))
          ) {
      ...
      const parents = new Set(
        components.map((component) => String(component.parentId ?? '')).filter(Boolean),
      );
    ...
    get filteredLivewireComponents() {
      ...
      if (search !== '') {
        const visible = new Set(
          this.matchingLivewireComponents.flatMap((component) => [
            String(component.id),
            ...component.ancestorIds,
          ]),
        );
    ...
    get selectedLivewireComponent() {
      return (
        this.livewireComponents.find((component) => component.id === this.livewireSelectedComponentId) ?? null
      );
    },
    ...
    livewireComponentIsSearchContext(component) {
      return (
        this.livewireSearch.trim() !== '' &&
        !this.matchingLivewireComponents.some((item) => item.id === component?.id)
      );
    },
