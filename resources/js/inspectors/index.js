...
export function createInspectorController(inspector, context) {
  const controller = controllers[inspector]?.(context);
  ...
  return composeState(controller, {
    ...
    destroyed: false,
    init() {
      shell.mountInspector(inspector, profileId, this, instance);
    },
    destroy() {
      this.destroyed = true;
      this.deactivate?.();
