...
export function createQueue(context) {
  ...
  return {
    refresh() {
      const payload = readInspectorPayload(this.$root, '[data-ndb-queue-payload]');
    ...
    visibleQueueCount: summary.inspector_counts?.queue ?? 0,
    ...
    applyQueueView() {
      ...
      const { visible, firstVisible, selectedVisible } = filterList(list?.children ?? [], {
        ...
        matches: (item) => {
          ...
          return (
            activity !== undefined && matchesFilter && (search === '' || activity.search.includes(search))
          );
        },
      });
