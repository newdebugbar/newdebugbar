  ...
  return {
    ...
    initializeQueries(records) {
      const previous = new Map(
        this.queryRecords.flatMap((record) => (record.executions ?? []).map((query) => [query.execution, query])),
      );
    ...
    receiveQueryExplain(detail = {}) {
      if (this.destroyed || this.profileId !== shell.summary.id || detail.profileId !== this.profileId) return;
